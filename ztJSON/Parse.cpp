...
namespace ztJSON {
	...
	void json_parse::skip_garbage() {
		skip_whitespace();
		bool comment_sign = false;
		do {
			comment_sign = skip_commits();
			skip_whitespace();
		} while (comment_sign);
	}

	long json_parse::parse_hex(const std::string& from) {
		long ret = 0;
		for (int i = 0; i <= 3; i++) {
			char ch = from[i];
			assert((ch >= '0'&&ch <= '9')
...
// 	
	void json_parse::encode_utf8(long code_point, std::string& ret) {
		if (code_point < 0) {
			return;
	...
	std::string json_parse::parse_string() {
		...
		for (;;) {
			...
			if (ch == 'u' || ch == 'U') {
				...
				if (high_surrogate >= 0xD800 && high_surrogate <= 0xD8FF) {
					if (str[i++] != '\\') {
					...
					high_surrogate = 0x10000 + (((high_surrogate - 0xD800) << 10) | (low_surrogate - 0xDC00));
					encode_utf8(code_point, ret);
				} else {
					encode_utf8(high_surrogate, ret);
				}
				i += 4;
				continue;
			}
